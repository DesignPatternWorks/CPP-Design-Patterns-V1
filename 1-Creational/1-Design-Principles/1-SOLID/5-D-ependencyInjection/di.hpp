...
struct join<type_list<Ts...>, type_list<T1s...>, type_list<T2s...>, type_list<T3s...>, type_list<T4s...>, type_list<T5s...>, type_list<T6s...>,
    ...
    : join<type_list<Ts..., T1s..., T2s..., T3s..., T4s..., T5s..., T6s..., T7s..., T8s..., T9s..., T10s..., T11s..., T12s..., T13s..., T14s...,
                     T15s..., T16s...>,
           Us...> {};
...
#endif
template <class T, class... TArgs>
decltype(::boost::di::v1_1_0::aux::declval<T>().operator()(::boost::di::v1_1_0::aux::declval<TArgs>()...), ::boost::di::v1_1_0::aux::true_type())
...
struct get_not_supported<T, TDeps...> : aux::conditional<aux::is_a<core::injector_base, T>::value || aux::is_a<core::dependency_base, T>::value,
...
using get_any_of_error =
    aux::conditional_t<aux::is_same<aux::bool_list<aux::always<Ts>::value...>, aux::bool_list<aux::is_same<aux::true_type, Ts>::value...>>::value,
...
auto boundable_impl(I&&, T &&)
    -> aux::conditional_t<aux::is_same<T, aux::decay_t<T>>::value, boundable_impl__<I, T>, typename type_<T>::has_disallowed_qualifiers>;
template <class I, class T>
...
template <class T, class... Ts>
auto boundable_impl(concepts::any_of<Ts...>&&, T &&) -> get_any_of_error<decltype(boundable_impl(aux::declval<Ts>(), aux::declval<T>()))...>;
template <class... TDeps>
...
template <class T>
typename scope<T>::template requires_<typename scope<_, _>::is_referable, typename scope<_, _>::try_create, typename scope<_, _>::create>
scopable_impl(...);
template <class T>
...
template <class T>
struct is_injectable<T, ::boost::di::v1_1_0::aux::valid_t<typename T::boost_di_inject__>> : ::boost::di::v1_1_0::aux::true_type {};
struct direct {};
struct uniform {};
...
struct array_impl : _ {
  using boost_di_inject__ = aux::type_list<Ts...>;
  explicit array_impl(type_traits::remove_named_t<Ts>&&... args) : array{static_cast<type_traits::remove_named_t<Ts>&&>(args)...} {}
  T array[sizeof...(Ts)];
};
template <class T, class... Ts>
struct array<T(), Ts...> : T {
  ...
  explicit array(array_t&& a) : T(std::move_iterator<value_type*>(a.array), std::move_iterator<value_type*>(a.array + sizeof...(Ts))) {}
};
template <class T>
struct array<T()> : T {
  using boost_di_inject__ = aux::type_list<>;
...
struct is_expr
    : aux::integral_constant<bool, aux::is_invocable<TGiven, typename TProvider::injector, Ts...>::value && !has_result_type<TGiven>::value> {};
}  // namespace detail
template <class T>
struct wrapper {
  inline operator T() noexcept { return static_cast<T&&>(object); }
    ...
    class injector_impl : injector__<Ts>... {
      ...
      struct create<named<TName, T>, aux::true_type> {
        static T impl(const injector__<named<TName, T>>* object) noexcept {
          ...
          return static_cast<const core::injector__<TInjector>&>(static_cast<const injector_impl*>(object)->injector_).create_successful_impl(type{});
        }
      };
      template <class TName, class T>
...
class dependency : dependency_base,
 ...
 public:
  ...
  auto to(T&& object) noexcept {
    using dependency = dependency<scopes::instance, deduce_traits_t<TExpected, T>, typename ref_traits<T>::type, TName, TPriority>;
  ...
  auto to(T&& object) noexcept {
    using dependency =
        dependency<scopes::instance, deduce_traits_t<concepts::any_of<TExpected, TConcept>, T>, typename ref_traits<T>::type, TName, TPriority>;
    return dependency{static_cast<T&&>(object)};
  }
  template <class TConcept, class T, __BOOST_DI_REQUIRES(externable<T>::value&& aux::is_callable<T>::value) = 0>
  auto to(T&& object) noexcept {
  ...
  auto operator[](const override&) noexcept { return dependency<TScope, TExpected, TGiven, TName, override>{static_cast<dependency&&>(*this)}; }
 ...
 public:
  ...
  class scope<_, T, aux::true_type> {
   public:
    ...
    static decltype(wrappers::shared<singleton, T_>{std::shared_ptr<T_>{std::shared_ptr<T_>{aux::declval<TProvider>().get()}}}) try_create(
        const TProvider&);
    template <class, class, class TProvider>
    auto create(const TProvider& provider) {
...
struct binder {
  ...
  static decltype(auto) resolve_impl(aux::pair<TConcept, TDependency<TScope, TExpected, TGiven, TName, override>>* dep) noexcept {
...
struct wrapper_impl<T, TWrapper<TScope, T_, Ts...>, __BOOST_DI_REQUIRES(!aux::is_convertible<TWrapper<TScope, T_, Ts...>, T>::value)> {
  inline operator T() noexcept { return typename concepts::scoped<TScope, aux::remove_qualifiers_t<T_>>::template is_not_convertible_to<T>{}; }
...
class injector : injector_base, public pool<bindings_t<TDeps...>> {
 ...
 protected:
  ...
  struct is_creatable {
    ...
    using ctor_t = typename type_traits::ctor_traits__<binder::resolve_template_t<injector, typename dependency_t::given>, T>::type;
 ...
 public:
  ...
  template <template <class...> class T,
            __BOOST_DI_REQUIRES(is_creatable<binder::resolve_template_t<injector, aux::identity<T<>>>, no_name, aux::true_type>::value) = 0>
 ...
 private:
  ...
  auto create_impl__() const {
    ...
    using wrapper_t = decltype(static_cast<dependency__<dependency_t>&>(dependency).template create<T, TName>(provider_t{this}));
    using ctor_args_t = typename ctor_t::second::second;
...
inline auto make_injector(TDeps... args) noexcept {
  return __BOOST_DI_MAKE_INJECTOR(core::injector<TConfig, decltype(((TConfig*)0)->policies((concepts::injector<TConfig>*)0)), TDeps...>{
...
struct or_ : detail::type_op {
  template <class TArg>
  struct apply : aux::integral_constant<bool, !aux::is_same<aux::bool_list<detail::apply_impl<Ts>::template apply<TArg>::value...>,
...
#define __BOOST_DI_REPEAT_8(m, ...) \
  m(0, __VA_ARGS__) m(1, __VA_ARGS__) m(2, __VA_ARGS__) m(3, __VA_ARGS__) m(4, __VA_ARGS__) m(5, __VA_ARGS__) m(6, __VA_ARGS__) m(7, __VA_ARGS__)
...
#define __BOOST_DI_INJECT_TRAITS_IMPL_1(...)                                                                                         \
  ...
  using type __BOOST_DI_UNUSED = ::boost::di::v1_1_0::detail::combine_t<::boost::di::v1_1_0::aux::function_traits_t<decltype(ctor)>, \
                                                                        ::boost::di::v1_1_0::aux::function_traits_t<decltype(name)>>;
#define __BOOST_DI_INJECT_TRAITS_EMPTY_IMPL(...) using boost_di_inject__ __BOOST_DI_UNUSED = ::boost::di::v1_1_0::aux::type_list<>
