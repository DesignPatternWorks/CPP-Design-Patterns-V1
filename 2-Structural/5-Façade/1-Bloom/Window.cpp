    ...
    for (size_t x = 0; x < cx; ++x) {
      for (auto v : viewports) {
        ...
        if (c) {
          ...
          text_renderer->DrawText(boost::lexical_cast<std::string>(c).c_str(), xx, yy, sx,
                                  sy);
        }
      }
    }
...
void Window::Show() {
  while (!glfwWindowShouldClose(handle)) {
    ...
    FT_Set_Pixel_Sizes(text_renderer->face, 0, 18);
...
void Window::KeyFunction(GLFWwindow* handle, int key, int scancode, int action,
                         int mods) {
  ...
  std::string s{};
  s += std::string("key is ") + boost::lexical_cast<std::string>(key);
  s += std::string(", modifier is ") + boost::lexical_cast<std::string>(mods);
  if (action == GLFW_PRESS)
    s += " pressed";
  else if (action == GLFW_RELEASE)
    s += " released";
  self->buffers[0]->add_string(s);
}
