...
class MenuBar : public Area, public MenuItemContainer {
 public:
  MenuBar(uint32_t width, uint32_t height) : Area{width, height} {}

  std::vector<std::shared_ptr<MenuItem>>& get_items() override { return items; }
...
class MenuBar::MenuBarBuilder {
 ...
 public:
  MenuBarBuilder& add(std::string text, std::function<bool(MenuItem&)> handler,
